...
#BT_RST default:Reset
echo 144 > /sys/class/gpio/export
chmod 777 /sys/class/gpio/gpio144
chmod 777 /sys/class/gpio/gpio144/*
echo low > /sys/class/gpio/gpio144/direction

#add jvckw middleware for bonjour
if [ -f /opt/jkpf/bin/mdnsd ]; then
	/opt/jkpf/bin/mdnsd &
fi

ln -s /dev/null /dev/ndda

/opt/jkpf/system_init.sh &
...
=jk_applications.sh= is run at runlevel 4 and runlevel 5 at priority 93
...
it calls =/opt/jkpf/system_init.sh=


=system_init.sh= is as:

in path, =Solutions/System/IPF/script/system_init.sh=
#+BEGIN_SRC shell
#!/bin/sh
...
# launch jk system
if [ -f /opt/jkpf/bin/System ]; then
	/opt/jkpf/bin/System > $OUTPUT 2>&1 &
fi
#+END_SRC

in =system_init.sh=, it calls =/opt/jkpf/bin/System=, which start up all the applications
